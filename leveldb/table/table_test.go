...
var _ = testutil.Defer(func() {
	Describe("Table", func() {
		Describe("approximate offset test", func() {
			...
			// Building the table.
			tw := NewWriter(buf, o, nil, 0, new(Stats))
			...
			It("Should be able to approximate offset of a key correctly", func() {
				...
				tr, err := NewReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), storage.FileDesc{}, nil, nil, o, new(Stats))
				Expect(err).ShouldNot(HaveOccurred())
		...
		Describe("read test", func() {
			Build := func(kv testutil.KeyValue) testutil.DB {
				...
				// Opening the table.
				tr, _ := NewReader(bytes.NewReader(buf.Bytes()), int64(buf.Len()), storage.FileDesc{}, nil, nil, o, new(Stats))
